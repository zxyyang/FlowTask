import { p as $, c as w, d as J, am as Q, h as M, l as x, x as j } from "./mermaid.core.ZiMYvjlI.js";
import { c as at } from "./createText-aebacdfe.4tdR55WE.js";
import { l as nt } from "./line.3WL5MWbk.js";
import { g as lt } from "./step.sPVUIIxm.js";
const ct = (a, t, r, n) => {
  t.forEach((e) => {
    ut[e](a, r, n);
  });
...
function mt(a) {
  const t = M(document.createElementNS("http://www.w3.org/2000/svg", "foreignObject")), r = t.append("xhtml:div"), n = a.label, e = a.isNode ? "nodeLabel" : "edgeLabel";
  return r.html(
...
const kt = (a, t, r, n) => {
  let e = a || "";
  if (typeof e == "object" && (e = e[0]), $(w().flowchart.htmlLabels)) {
...
}, I = kt, _ = async (a, t, r, n) => {
  let e;
  const i = t.useHtmlLabels || $(w().flowchart.htmlLabels);
  ...
  const h = l.node();
  let o;
  t.labelType === "markdown" ? o = at(l, J(Q(c), w()), {
    useHtmlLabels: i,
  ...
  let f = o.getBBox();
  const y = t.padding / 2;
  if ($(w().flowchart.htmlLabels)) {
    const p = o.children[0], u = M(o), k = p.getElementsByTagName("img");
    if (k) {
          ...
          (d) => new Promise((b) => {
            function B() {
              if (d.style.display = "flex", d.style.flexDirection = "column", g) {
                ...
                d.style.width = parseInt(C, 10) * X + "px";
              } else
                d.style.width = "100%";
              b(d);
            }
            setTimeout(() => {
              d.complete && B();
            }), d.addEventListener("error", B), d.addEventListener("load", B);
          })
        )
      );
    }
...
function et(a, t, r, n) {
  var e = a.x, i = a.y, s = e - n.x, l = i - n.y, c = Math.sqrt(t * t * l * l + r * r * s * s), h = Math.abs(t * r * s / c);
  n.x < e && (h = -h);
  var o = Math.abs(t * r * l / c);
  return n.y < i && (o = -o), { x: e + h, y: i + o };
}
function Lt(a, t, r) {
  return et(a, t, t, r);
}
function St(a, t, r, n) {
  var e, i, s, l, c, h, o, f, y, p, u, k, g, d, b;
  if (e = t.y - a.y, s = a.x - t.x, c = t.x * a.y - a.x * t.y, y = e * r.x + s * r.y + c, p = e * n.x + s * n.y + c, !(y !== 0 && p !== 0 && q(y, p)) && (i = n.y - r.y, l = r.x - n.x, h = n.x * r.y - r.x * n.y, o = i * a.x + l * a.y + h, f = i * t.x + l * t.y + h, !(o !== 0 && f !== 0 && q(o, f)) && (u = e * l - i * s, u !== 0)))
    return k = Math.abs(u / 2), g = s * h - l * c, d = g < 0 ? (g - k) / u : (g + k) / u, g = i * c - e * h, b = g < 0 ? (g - k) / u : (g + k) / u, { x: d, y: b };
}
function q(a, t) {
  return a * t > 0;
}
function Et(a, t, r) {
  ...
  for (var c = n - a.width / 2 - s, h = e - a.height / 2 - l, o = 0; o < t.length; o++) {
    var f = t[o], y = t[o < t.length - 1 ? o + 1 : 0], p = St(
      a,
      r,
      { x: c + f.x, y: h + f.y },
      { x: c + y.x, y: h + y.y }
  ...
  return i.length ? (i.length > 1 && i.sort(function(u, k) {
    var g = u.x - r.x, d = u.y - r.y, b = Math.sqrt(g * g + d * d), B = k.x - r.x, C = k.y - r.y, X = Math.sqrt(B * B + C * C);
    return b < X ? -1 : b === X ? 0 : 1;
  }), i[0]) : a;
}
const Bt = (a, t) => {
  var r = a.x, n = a.y, e = t.x - r, i = t.y - n, s = a.width / 2, l = a.height / 2, c, h;
  return Math.abs(i) * s > Math.abs(e) * l ? (i < 0 && (l = -l), c = i === 0 ? 0 : l * e / i, h = l) : (e < 0 && (s = -s), c = s, h = e === 0 ? 0 : s * i / e), { x: r + c, y: n + h };
}, Mt = Bt, m = {
  node: vt,
  circle: Lt,
  ellipse: et,
  ...
  const { shapeSvg: n, bbox: e, halfPadding: i } = await _(
    a,
    t,
    "node " + t.classes,
    !0
...
}, Ct = _t, K = (a) => a ? " " + a : "", R = (a, t) => `${t || "node default"}${K(a.classes)} ${K(
  a.class
)}`, G = async (a, t) => {
  const { shapeSvg: r, bbox: n } = await _(
    a,
    t,
    R(t, void 0),
    !0
  ...
  const c = N(r, s, s, l);
  return c.attr("style", t.style), v(t, c), t.intersect = function(h) {
    return x.warn("Intersect called"), m.polygon(t, l, h);
  }, r;
}, Tt = (a, t) => {
  ...
  return h.attr("style", t.style), v(t, h), t.intersect = function(o) {
    return m.polygon(t, c, o);
  }, r;
}, It = async (a, t) => {
  const { shapeSvg: r, bbox: n } = await _(
    a,
  ...
  return N(r, e, i, s).attr("style", t.style), t.width = e + i, t.height = i, t.intersect = function(c) {
    return m.polygon(t, s, c);
  }, r;
}, $t = async (a, t) => {
  const { shapeSvg: r, bbox: n } = await _(a, t, R(t), !0), e = n.width + t.padding, i = n.height + t.padding, s = [
    { x: -2 * i / 6, y: 0 },
    { x: e - i / 6, y: 0 },
    { x: e + 2 * i / 6, y: -i },
    { x: i / 6, y: -i }
  ...
  const { shapeSvg: r, bbox: n } = await _(
    ...
    R(t, void 0),
    !0
  ), e = n.width + t.padding, i = e / 2, s = i / (2.5 + e / 50), l = n.height + s + t.padding, c = "M 0," + s + " a " + i + "," + s + " 0,0,0 " + e + " 0 a " + i + "," + s + " 0,0,0 " + -e + " 0 l 0," + l + " a " + i + "," + s + " 0,0,0 " + e + " 0 l 0," + -l, h = r.attr("label-offset-y", s).insert("path", ":first-child").attr("style", t.style).attr("d", c).attr("transform", "translate(" + -e / 2 + "," + -(l / 2 + s) + ")");
  return v(t, h), t.intersect = function(o) {
    const f = m.rect(t, o), y = f.x - t.x;
    if (i != 0 && (Math.abs(y) < t.width / 2 || Math.abs(y) == t.width / 2 && Math.abs(f.y - t.y) > t.height / 2 - s)) {
      let p = s * s * (1 - y * y / (i * i));
      p != 0 && (p = Math.sqrt(p)), p = s - p, o.y - t.y > 0 && (p = -p), f.y += p;
    }
    return f;
  }, r;
}, Wt = async (a, t) => {
  const { shapeSvg: r, bbox: n, halfPadding: e } = await _(
    a,
    t,
  ...
  if (i.attr("class", "basic label-container").attr("style", t.style).attr("rx", t.rx).attr("ry", t.ry).attr("x", -n.width / 2 - e).attr("y", -n.height / 2 - e).attr("width", s).attr("height", l), t.props) {
    ...
    t.props.borders && (st(i, t.props.borders, s, l), c.delete("borders")), c.forEach((h) => {
      x.warn(`Unknown node property ${h}`);
    });
  }
  ...
  const { shapeSvg: r } = await _(a, t, "label", !0);
...
const jt = (a, t) => {
  ...
  const h = s.node().appendChild(I(c, t.labelStyle, !0, !0));
  let o = { width: 0, height: 0 };
  if ($(w().flowchart.htmlLabels)) {
    const k = h.children[0], g = M(h);
    o = k.getBoundingClientRect(), g.attr("width", o.width), g.attr("height", o.height);
  }
  x.info("Text 2", l);
  ...
  return M(p).attr(
    ...
    (o.width > y.width ? 0 : (y.width - o.width) / 2) + ", " + (y.height + u + 5) + ")"
  ), M(h).attr(
    "transform",
    "translate( " + // (titleBox.width - bbox.width) / 2 +
    (o.width < y.width ? 0 : -(y.width - o.width) / 2) + ", 0)"
  ), o = s.node().getBBox(), s.attr(
    "transform",
    "translate(" + -o.width / 2 + ", " + (-o.height / 2 - u + 3) + ")"
  ), e.attr("class", "outer title-state").attr("x", -o.width / 2 - u).attr("y", -o.height / 2 - u).attr("width", o.width + t.padding).attr("height", o.height + t.padding), i.attr("class", "divider").attr("x1", -o.width / 2 - u).attr("x2", o.width / 2 + u).attr("y1", -o.height / 2 - u + y.height + u).attr("y2", -o.height / 2 - u + y.height + u), v(t, e), t.intersect = function(k) {
    return m.rect(t, k);
  }, n;
}, At = async (a, t) => {
  const { shapeSvg: r, bbox: n } = await _(
  ...
  const s = a.insert("g").attr("class", i).attr("id", t.domId || t.id), l = s.insert("rect", ":first-child"), c = s.insert("line"), h = s.insert("line");
  let o = 0, f = n;
  const y = s.insert("g").attr("class", "label");
  let p = 0;
  const u = t.classData.annotations && t.classData.annotations[0], k = t.classData.annotations[0] ? "«" + t.classData.annotations[0] + "»" : "", g = y.node().appendChild(I(k, t.labelStyle, !0, !0));
  let d = g.getBBox();
  if ($(w().flowchart.htmlLabels)) {
    const L = g.children[0], S = M(g);
    d = L.getBoundingClientRect(), S.attr("width", d.width), S.attr("height", d.height);
  }
  t.classData.annotations[0] && (f += d.height + n, o += d.width);
  ...
  t.classData.members.forEach((L) => {
    ...
    let T = H.getBBox();
    if ($(w().flowchart.htmlLabels)) {
      const F = H.children[0], D = M(H);
      T = F.getBoundingClientRect(), D.attr("width", T.width), D.attr("height", T.height);
    }
    T.width > o && (o = T.width), f += T.height + n, X.push(H);
  }), f += e;
  const V = [];
  if (t.classData.methods.forEach((L) => {
    ...
    T.width > o && (o = T.width), f += T.height + n, V.push(H);
  }), f += e, u) {
    let L = (o - d.width) / 2;
    M(g).attr(
      "transform",
      "translate( " + (-1 * o / 2 + L) + ", " + -1 * f / 2 + ")"
    ), p = d.height + n;
  }
  let it = (o - C.width) / 2;
  return M(B).attr(
    "transform",
    "translate( " + (-1 * o / 2 + it) + ", " + (-1 * f / 2 + p) + ")"
  ), p += C.height + n, c.attr("class", "divider").attr("x1", -o / 2 - r).attr("x2", o / 2 + r).attr("y1", -f / 2 - r + e + p).attr("y2", -f / 2 - r + e + p), p += e, X.forEach((L) => {
    M(L).attr(
      "transform",
      "translate( " + -o / 2 + ", " + (-1 * f / 2 + p + e / 2) + ")"
    );
    const S = L == null ? void 0 : L.getBBox();
    p += ((S == null ? void 0 : S.height) ?? 0) + n;
  }), p += e, h.attr("class", "divider").attr("x1", -o / 2 - r).attr("x2", o / 2 + r).attr("y1", -f / 2 - r + e + p).attr("y2", -f / 2 - r + e + p), p += e, V.forEach((L) => {
    M(L).attr(
      "transform",
      "translate( " + -o / 2 + ", " + (-1 * f / 2 + p) + ")"
    );
    const S = L == null ? void 0 : L.getBBox();
    p += ((S == null ? void 0 : S.height) ?? 0) + n;
  }), l.attr("class", "outer title-state").attr("x", -o / 2 - r).attr("y", -(f / 2) - r).attr("width", o + t.padding).attr("height", f + t.padding), v(t, l), t.intersect = function(L) {
    return m.rect(t, L);
  }, s;
}, tt = {
  rhombus: G,
...
const lr = () => {
  ...
  const r = $(w().flowchart.htmlLabels), n = t.labelType === "markdown" ? at(a, t.label, {
    style: t.labelStyle,
    useHtmlLabels: r,
    addSvgBackground: !0
  ...
  let l;
  if (t.startLabelLeft) {
    const c = I(t.startLabelLeft, t.labelStyle), h = a.insert("g").attr("class", "edgeTerminals"), o = h.insert("g").attr("class", "inner");
    l = o.node().appendChild(c);
    const f = c.getBBox();
    o.attr("transform", "translate(" + -f.width / 2 + ", " + -f.height / 2 + ")"), E[t.id] || (E[t.id] = {}), E[t.id].startLeft = h, U(l, t.startLabelLeft);
  }
  if (t.startLabelRight) {
    const c = I(t.startLabelRight, t.labelStyle), h = a.insert("g").attr("class", "edgeTerminals"), o = h.insert("g").attr("class", "inner");
    l = h.node().appendChild(c), o.node().appendChild(c);
    const f = c.getBBox();
    o.attr("transform", "translate(" + -f.width / 2 + ", " + -f.height / 2 + ")"), E[t.id] || (E[t.id] = {}), E[t.id].startRight = h, U(l, t.startLabelRight);
  }
  if (t.endLabelLeft) {
    const c = I(t.endLabelLeft, t.labelStyle), h = a.insert("g").attr("class", "edgeTerminals"), o = h.insert("g").attr("class", "inner");
    l = o.node().appendChild(c);
    const f = c.getBBox();
    o.attr("transform", "translate(" + -f.width / 2 + ", " + -f.height / 2 + ")"), h.node().appendChild(c), E[t.id] || (E[t.id] = {}), E[t.id].endLeft = h, U(l, t.endLabelLeft);
  }
  if (t.endLabelRight) {
    const c = I(t.endLabelRight, t.labelStyle), h = a.insert("g").attr("class", "edgeTerminals"), o = h.insert("g").attr("class", "inner");
    l = o.node().appendChild(c);
    const f = c.getBBox();
    o.attr("transform", "translate(" + -f.width / 2 + ", " + -f.height / 2 + ")"), h.node().appendChild(c), E[t.id] || (E[t.id] = {}), E[t.id].endRight = h, U(l, t.endLabelRight);
  }
  return n;
};
function U(a, t) {
...
const or = (a, t) => {
  x.info("Moving label abc78 ", a.id, a.label, Z[a.id]);
  let r = t.updatedPath ? t.updatedPath : t.originalPath;
  if (a.label) {
    const n = Z[a.id];
    let e = a.x, i = a.y;
    if (r) {
      const s = j.calcLabelPosition(r);
      x.info(
  ...
  if (a.startLabelLeft) {
    ...
    let e = a.x, i = a.y;
    if (r) {
      const s = j.calcTerminalLabelPosition(a.arrowTypeStart ? 10 : 0, "start_left", r);
      e = s.x, i = s.y;
    }
    n.attr("transform", "translate(" + e + ", " + i + ")");
  }
  if (a.startLabelRight) {
    ...
    let e = a.x, i = a.y;
    if (r) {
      const s = j.calcTerminalLabelPosition(
  ...
  if (a.endLabelLeft) {
    ...
    let e = a.x, i = a.y;
    if (r) {
      const s = j.calcTerminalLabelPosition(a.arrowTypeEnd ? 10 : 0, "end_left", r);
      e = s.x, i = s.y;
    }
    n.attr("transform", "translate(" + e + ", " + i + ")");
  }
  if (a.endLabelRight) {
    ...
    let e = a.x, i = a.y;
    if (r) {
      const s = j.calcTerminalLabelPosition(a.arrowTypeEnd ? 10 : 0, "end_right", r);
      e = s.x, i = s.y;
    }
    n.attr("transform", "translate(" + e + ", " + i + ")");
  }
  ...
  const c = a.height / 2, h = Math.abs(t.y - r.y), o = Math.abs(t.x - r.x);
  if (Math.abs(e - t.y) * s > Math.abs(n - t.x) * c) {
    let f = r.y < t.y ? t.y - c - e : e - c - t.y;
    l = o * f / h;
    const y = {
      x: r.x < t.x ? r.x + l : r.x - o + l,
      y: r.y < t.y ? r.y + h - f : r.y - h + f
    };
    return l === 0 && (y.x = t.x, y.y = t.y), o === 0 && (y.x = t.x), h === 0 && (y.y = t.y), x.warn(`abc89 topp/bott calc, Q ${h}, q ${f}, R ${o}, r ${l}`, y), y;
  } else {
    r.x < t.x ? l = t.x - s - n : l = n - s - t.x;
    let f = h * l / o, y = r.x < t.x ? r.x + o - l : r.x - o + l, p = r.y < t.y ? r.y + f : r.y - f;
    return x.warn(`sides calc abc89, Q ${h}, q ${f}, R ${o}, r ${l}`, { _x: y, _y: p }), l === 0 && (y = t.x, p = t.y), o === 0 && (y = t.x), h === 0 && (p = t.y), { x: y, y: p };
  }
}, rt = (a, t) => {
  ...
  const h = i.node(t.v);
  var o = i.node(t.w);
  x.info("abc88 InsertEdge: ", r), o.intersect && h.intersect && (l = l.slice(1, r.points.length - 1), l.unshift(h.intersect(l[0])), x.info(
    "Last point",
    l[l.length - 1],
    o,
    o.intersect(l[l.length - 1])
  ), l.push(o.intersect(l[l.length - 1]))), r.toCluster && (x.info("to cluster abc88", n[r.toCluster]), l = rt(r.points, n[r.toCluster].node), c = !0), r.fromCluster && (x.info("from cluster abc88", n[r.fromCluster]), l = rt(l.reverse(), n[r.fromCluster].node).reverse(), c = !0);
  const f = l.filter((C) => !Number.isNaN(C.y));
  let y = lt;
  r.curve && (e === "graph" || e === "flowchart") && (y = r.curve);
  const { x: p, y: u } = Jt(r), k = nt().x(p).y(u).curve(y);
  let g;
  switch (r.thickness) {
    case "normal":
  ...
  let B = {};
  return c && (B.updatedPath = l), B.originalPath = r.points, B;
};
export {
  er as a,
  cr as b,
