import { c as A, s as Mt, g as Ct, a as At, b as Ot, u as St, v as It, l as G, j as D, y as Gt, h as Pt, x as Nt, aq as Ht, ar as Bt } from "./mermaid.core.ZiMYvjlI.js";
import "./emojis.bYhmvyOg.js";
import "./step.sPVUIIxm.js";
import "../locales/en.js";
var mt = function() {
  var r = function(j, h, b, k) {
    for (b = b || {}, k = j.length; k--; b[j[k]] = h)
      ;
    return b;
  }, a = [1, 3], o = [1, 6], u = [1, 4], n = [1, 5], c = [2, 5], m = [1, 12], l = [5, 7, 13, 19, 21, 23, 24, 26, 28, 31, 36, 39, 46], E = [7, 13, 19, 21, 23, 24, 26, 28, 31, 36, 39], _ = [7, 12, 13, 19, 21, 23, 24, 26, 28, 31, 36, 39], i = [7, 13, 46], g = [1, 42], p = [1, 41], x = [7, 13, 29, 32, 34, 37, 46], f = [1, 55], d = [1, 56], y = [1, 57], H = [7, 13, 32, 34, 41, 46], w = {
    trace: function() {
    },
    yy: {},
    ...
    performAction: function(h, b, k, s, T, t, X) {
      var e = t.length - 1;
      switch (T) {
        case 2:
          return t[e];
    ...
    parse: function(h) {
      var b = this, k = [0], s = [], T = [null], t = [], X = this.table, e = "", rt = 0, pt = 0, Tt = 2, ft = 1, Lt = t.slice.call(arguments, 1), v = Object.create(this.lexer), Y = { yy: {} };
      for (var ct in this.yy)
        Object.prototype.hasOwnProperty.call(this.yy, ct) && (Y.yy[ct] = this.yy[ct]);
      v.setInput(h, Y.yy), Y.yy.lexer = v, Y.yy.parser = this, typeof v.yylloc > "u" && (v.yylloc = {});
      var ot = v.yylloc;
      t.push(ot);
      var Rt = v.options && v.options.ranges;
      typeof Y.yy.parseError == "function" ? this.parseError = Y.yy.parseError : this.parseError = Object.getPrototypeOf(this).parseError;
      function vt() {
        var q;
        return q = s.pop() || v.lex() || ft, typeof q != "number" && (q instanceof Array && (s = q, q = s.pop()), q = b.symbols_[q] || q), q;
      }
      for (var I, K, V, lt, W = {}, it, z, bt, st; ; ) {
        if (K = k[k.length - 1], this.defaultActions[K] ? V = this.defaultActions[K] : ((I === null || typeof I > "u") && (I = vt()), V = X[K] && X[K][I]), typeof V > "u" || !V.length || !V[0]) {
          var ht = "";
          st = [];
          for (it in X[K])
            this.terminals_[it] && it > Tt && st.push("'" + this.terminals_[it] + "'");
          v.showPosition ? ht = "Parse error on line " + (rt + 1) + `:
` + v.showPosition() + `
Expecting ` + st.join(", ") + ", got '" + (this.terminals_[I] || I) + "'" : ht = "Parse error on line " + (rt + 1) + ": Unexpected " + (I == ft ? "end of input" : "'" + (this.terminals_[I] || I) + "'"), this.parseError(ht, {
            text: v.match,
        ...
        switch (V[0]) {
          case 1:
            k.push(I), T.push(v.yytext), t.push(v.yylloc), k.push(V[1]), I = null, pt = v.yyleng, e = v.yytext, rt = v.yylineno, ot = v.yylloc;
            break;
          case 2:
            if (z = this.productions_[V[1]][1], W.$ = T[T.length - z], W._$ = {
              ...
              return lt;
            z && (k = k.slice(0, -1 * z * 2), T = T.slice(0, -1 * z), t = t.slice(0, -1 * z)), k.push(this.productions_[V[1]][0]), T.push(W.$), t.push(W._$), bt = X[k[k.length - 2]][k[k.length - 1]], k.push(bt);
            break;
          case 3:
            return !0;
        }
...
mt.parser = mt;
const Vt = mt;
let at = A().gitGraph.mainBranchName, Dt = A().gitGraph.mainBranchOrder, R = {}, S = null, Z = {};
Z[at] = { name: at, order: Dt };
let L = {};
L[at] = S;
let C = at, kt = "LR", U = 0;
function ut() {
...
const jt = function(r) {
  ...
  G.debug("Entering commit:", r, a, o, u), a = D.sanitizeText(a, A()), r = D.sanitizeText(r, A()), u = D.sanitizeText(u, A());
  const n = {
    id: a || U + "-" + ut(),
    message: r,
    seq: U++,
  ...
  S = n, R[n.id] = n, L[C] = n.id, G.debug("in pushCommit " + n.id);
}, Ft = function(r, a) {
  if (r = D.sanitizeText(r, A()), L[r] === void 0)
    L[r] = S != null ? S.id : null, Z[r] = { name: r, order: a ? parseInt(a, 10) : null }, yt(r), G.debug("in createBranch");
  else {
    let o = new Error(
  ...
  r = D.sanitizeText(r, A()), a = D.sanitizeText(a, A());
  const n = R[L[C]], c = R[L[r]];
  if (C === r) {
    ...
    let l = new Error(
      'Incorrect usage of "merge". Current branch (' + C + ")has no commits"
    );
    throw l.hash = {
      text: "merge " + r,
      token: "merge " + r,
  ...
  const m = {
    id: a || U + "-" + ut(),
    message: "merged branch " + r + " into " + C,
    seq: U++,
  ...
  S = m, R[m.id] = m, L[C] = m.id, G.debug(L), G.debug("in mergeBranch");
}, Wt = function(r, a, o) {
  if (G.debug("Entering cherryPick:", r, a, o), r = D.sanitizeText(r, A()), a = D.sanitizeText(a, A()), o = D.sanitizeText(o, A()), !r || R[r] === void 0) {
    let c = new Error(
  ...
  if (!a || R[a] === void 0) {
    ...
    if (c === void 0 || !c) {
      let l = new Error(
        'Incorrect usage of "cherry-pick". Current branch (' + C + ")has no commits"
      );
      throw l.hash = {
        text: "cherryPick " + r + " " + a,
        token: "cherryPick " + r + " " + a,
    ...
    const m = {
      id: U + "-" + ut(),
      message: "cherry-picked " + u + " into " + C,
      seq: U++,
    ...
    S = m, R[m.id] = m, L[C] = m.id, G.debug(L), G.debug("in cherryPick");
  }
}, yt = function(r) {
  if (r = D.sanitizeText(r, A()), L[r] === void 0) {
    let a = new Error(
...
const Xt = function() {
  ...
  R = {}, S = null;
  let r = A().gitGraph.mainBranchName, a = A().gitGraph.mainBranchOrder;
  L = {}, L[r] = null, Z = {}, Z[r] = { name: r, order: a }, C = r, U = 0, Gt();
}, Qt = function() {
...
const se = () => {
  ...
  const u = A().gitGraph, n = r.append("g").attr("class", "commit-bullets"), c = r.append("g").attr("class", "commit-labels");
  let m = 0;
  O === "TB" && (m = 30), Object.keys(a).sort((_, i) => a[_].seq - a[i].seq).forEach((_) => {
    const i = a[_], g = O === "TB" ? m + 10 : M[i.branch].pos, p = O === "TB" ? M[i.branch].pos : m + 10;
    if (o) {
      let x, f = i.customType !== void 0 && i.customType !== "" ? i.customType : i.type;
      switch (f) {
        case N.NORMAL:
          x = "commit-normal";
          break;
      ...
      if (f === N.HIGHLIGHT) {
        ...
        d.attr("x", p - 10), d.attr("y", g - 10), d.attr("height", 20), d.attr("width", 20), d.attr(
          "class",
          `commit ${i.id} commit-highlight${M[i.branch].index % F} ${x}-outer`
        ), n.append("rect").attr("x", p - 6).attr("y", g - 6).attr("height", 12).attr("width", 12).attr(
          "class",
          `commit ${i.id} commit${M[i.branch].index % F} ${x}-inner`
        );
      } else if (f === N.CHERRY_PICK)
        n.append("circle").attr("cx", p).attr("cy", g).attr("r", 10).attr("class", `commit ${i.id} ${x}`), n.append("circle").attr("cx", p - 3).attr("cy", g + 2).attr("r", 2.75).attr("fill", "#fff").attr("class", `commit ${i.id} ${x}`), n.append("circle").attr("cx", p + 3).attr("cy", g + 2).attr("r", 2.75).attr("fill", "#fff").attr("class", `commit ${i.id} ${x}`), n.append("line").attr("x1", p + 3).attr("y1", g + 1).attr("x2", p).attr("y2", g - 5).attr("stroke", "#fff").attr("class", `commit ${i.id} ${x}`), n.append("line").attr("x1", p - 3).attr("y1", g + 1).attr("x2", p).attr("y2", g - 5).attr("stroke", "#fff").attr("class", `commit ${i.id} ${x}`);
      else {
        const d = n.append("circle");
        if (d.attr("cx", p), d.attr("cy", g), d.attr("r", i.type === N.MERGE ? 9 : 10), d.attr(
          "class",
          `commit ${i.id} commit${M[i.branch].index % F}`
        ), f === N.MERGE) {
          const y = n.append("circle");
          y.attr("cx", p), y.attr("cy", g), y.attr("r", 6), y.attr(
            "class",
            `commit ${x} ${i.id} commit${M[i.branch].index % F}`
          );
        }
        f === N.REVERSE && n.append("path").attr("d", `M ${p - 5},${g - 5}L${p + 5},${g + 5}M${p - 5},${g + 5}L${p + 5},${g - 5}`).attr(
          "class",
          `commit ${x} ${i.id} commit${M[i.branch].index % F}`
        );
      }
    }
    if (O === "TB" ? tt[i.id] = { x: p, y: m + 10 } : tt[i.id] = { x: m + 10, y: g }, o) {
      if (i.type !== N.CHERRY_PICK && (i.customId && i.type === N.MERGE || i.type !== N.MERGE) && u.showCommitLabel) {
        const d = c.append("g"), y = d.insert("rect").attr("class", "commit-label-bkg"), H = d.append("text").attr("x", m).attr("y", g + 25).attr("class", "commit-label").text(i.id);
        let w = H.node().getBBox();
        if (y.attr("x", m + 10 - w.width / 2 - 2).attr("y", g + 13.5).attr("width", w.width + 2 * 2).attr("height", w.height + 2 * 2), O === "TB" && (y.attr("x", p - (w.width + 4 * 4 + 5)).attr("y", g - 12), H.attr("x", p - (w.width + 4 * 4)).attr("y", g + w.height - 12)), O !== "TB" && H.attr("x", m + 10 - w.width / 2), u.rotateCommitLabel)
          if (O === "TB")
            H.attr("transform", "rotate(-45, " + p + ", " + g + ")"), y.attr("transform", "rotate(-45, " + p + ", " + g + ")");
          else {
      ...
      if (i.tag) {
        const d = c.insert("polygon"), y = c.append("circle"), H = c.append("text").attr("y", g - 16).attr("class", "tag-label").text(i.tag);
        let w = H.node().getBBox();
        H.attr("x", m + 10 - w.width / 2);
        ...
        d.attr("class", "tag-label-bkg").attr(
          ...
          ${m + 10 + w.width / 2 + 4},${P + B + 2}
          ${m + 10 - w.width / 2 - 4},${P + B + 2}`
        ), y.attr("cx", m - w.width / 2 + 4 / 2).attr("cy", P).attr("r", 1.5).attr("class", "tag-hole"), O === "TB" && (d.attr("class", "tag-label-bkg").attr(
          "points",
          `
            ${p},${m + 2}
            ${p},${m - 2}
            ...
            ${p + 10 + w.width + 4},${m + B + 2}
            ${p + 10},${m + B + 2}`
        ).attr("transform", "translate(12,12) rotate(45, " + p + "," + m + ")"), y.attr("cx", p + 4 / 2).attr("cy", m).attr("transform", "translate(12,12) rotate(45, " + p + "," + m + ")"), H.attr("x", p + 5).attr("y", m + 3).attr("transform", "translate(14,14) rotate(45, " + p + "," + m + ")"));
      }
    }
    m += 50, m > et && (et = m);
  });
}, ae = (r, a, o) => Object.keys(o).filter((c) => o[c].branch === a.branch && o[c].seq > r.seq && o[c].seq < a.seq).length > 0, Q = (r, a, o = 0) => {
  ...
  const n = tt[a.id], c = tt[o.id], m = ae(a, o, u);
  let l = "", E = "", _ = 0, i = 0, g = M[o.branch].index, p;
  if (m) {
    l = "A 10 10, 0, 0, 0,", E = "A 10 10, 0, 0, 1,", _ = 10, i = 10, g = M[o.branch].index;
    const x = n.y < c.y ? Q(n.y, c.y) : Q(c.y, n.y), f = n.x < c.x ? Q(n.x, c.x) : Q(c.x, n.x);
    O === "TB" ? n.x < c.x ? p = `M ${n.x} ${n.y} L ${f - _} ${n.y} ${E} ${f} ${n.y + i} L ${f} ${c.y - _} ${l} ${f + i} ${c.y} L ${c.x} ${c.y}` : p = `M ${n.x} ${n.y} L ${f + _} ${n.y} ${l} ${f} ${n.y + i} L ${f} ${c.y - _} ${E} ${f - i} ${c.y} L ${c.x} ${c.y}` : n.y < c.y ? p = `M ${n.x} ${n.y} L ${n.x} ${x - _} ${l} ${n.x + i} ${x} L ${c.x - _} ${x} ${E} ${c.x} ${x + i} L ${c.x} ${c.y}` : p = `M ${n.x} ${n.y} L ${n.x} ${x + _} ${E} ${n.x + i} ${x} L ${c.x - _} ${x} ${l} ${c.x} ${x - i} L ${c.x} ${c.y}`;
  } else
    O === "TB" ? (n.x < c.x && (l = "A 20 20, 0, 0, 0,", E = "A 20 20, 0, 0, 1,", _ = 20, i = 20, g = M[o.branch].index, p = `M ${n.x} ${n.y} L ${c.x - _} ${n.y} ${E} ${c.x} ${n.y + i} L ${c.x} ${c.y}`), n.x > c.x && (l = "A 20 20, 0, 0, 0,", E = "A 20 20, 0, 0, 1,", _ = 20, i = 20, g = M[a.branch].index, p = `M ${n.x} ${n.y} L ${n.x} ${c.y - _} ${E} ${n.x - i} ${c.y} L ${c.x} ${c.y}`), n.x === c.x && (g = M[a.branch].index, p = `M ${n.x} ${n.y} L ${n.x + _} ${n.y} ${l} ${n.x + i} ${c.y + _} L ${c.x} ${c.y}`)) : (n.y < c.y && (l = "A 20 20, 0, 0, 0,", _ = 20, i = 20, g = M[o.branch].index, p = `M ${n.x} ${n.y} L ${n.x} ${c.y - _} ${l} ${n.x + i} ${c.y} L ${c.x} ${c.y}`), n.y > c.y && (l = "A 20 20, 0, 0, 0,", _ = 20, i = 20, g = M[a.branch].index, p = `M ${n.x} ${n.y} L ${c.x - _} ${n.y} ${l} ${c.x} ${n.y - i} L ${c.x} ${c.y}`), n.y === c.y && (g = M[a.branch].index, p = `M ${n.x} ${n.y} L ${n.x} ${c.y - _} ${l} ${n.x + i} ${c.y} L ${c.x} ${c.y}`));
  r.append("path").attr("d", p).attr("class", "arrow arrow" + g % F);
}, ce = (r, a) => {
  ...
  const o = A().gitGraph, u = r.append("g");
  a.forEach((n, c) => {
    const m = c % F, l = M[n.name].pos, E = u.append("line");
    E.attr("x1", 0), E.attr("y1", l), E.attr("x2", et), E.attr("y2", l), E.attr("class", "branch branch" + m), O === "TB" && (E.attr("y1", 30), E.attr("x1", l), E.attr("y2", et), E.attr("x2", l)), nt.push(l);
    let _ = n.name;
    const i = wt(_), g = u.insert("rect"), x = u.insert("g").attr("class", "branchLabel").insert("g").attr("class", "label branch-label" + m);
    x.node().appendChild(i);
    let f = i.getBBox();
    g.attr("class", "branchLabelBkg label" + m).attr("rx", 4).attr("ry", 4).attr("x", -f.width - 4 - (o.rotateCommitLabel === !0 ? 30 : 0)).attr("y", -f.height / 2 + 8).attr("width", f.width + 18).attr("height", f.height + 4), x.attr(
      "transform",
      "translate(" + (-f.width - 14 - (o.rotateCommitLabel === !0 ? 30 : 0)) + ", " + (l - f.height / 2 - 1) + ")"
    ), O === "TB" && (g.attr("x", l - f.width / 2 - 10).attr("y", 0), x.attr("transform", "translate(" + (l - f.width / 2 - 5) + ", 0)")), O !== "TB" && g.attr("transform", "translate(-19, " + (l - f.height / 2) + ")");
  });
}, le = function(r, a, o, u) {
  se();
  const n = A(), c = n.gitGraph;
  G.debug("in gitgraph renderer", r + `
`, "id:", a, o), J = u.db.getCommits();
  const m = u.db.getBranchesAsObjArray();
  O = u.db.getDirection();
  const l = Pt(`[id="${a}"]`);
  let E = 0;
  m.forEach((_, i) => {
    const g = wt(_.name), p = l.append("g"), x = p.insert("g").attr("class", "branchLabel"), f = x.insert("g").attr("class", "label branch-label");
    f.node().appendChild(g);
    let d = g.getBBox();
    M[_.name] = { pos: E, index: i }, E += 50 + (c.rotateCommitLabel ? 40 : 0) + (O === "TB" ? d.width / 2 : 0), f.remove(), x.remove(), p.remove();
  }), dt(l, J, !1), c.showBranches && oe(l, m), ce(l, J), dt(l, J, !0), Nt.insertTitle(
    l,
    "gitTitleText",
    c.titleTopMargin,
